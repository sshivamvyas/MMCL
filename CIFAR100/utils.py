...
def get_dataset(dataset_name, dataset_location, pair=True):
  ...
  #  Here, change the 5000/500/500 as needed.
    train_indices = random.sample(range(len(train_data)), min(10000, len(train_data)))
    memory_indices = random.sample(range(len(memory_data)), min(1000, len(memory_data)))
    test_indices = random.sample(range(len(test_data)), min(1000, len(test_data)))
