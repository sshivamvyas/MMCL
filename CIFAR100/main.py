...
def test(net, memory_data_loader, test_data_loader):
    ...
    with torch.no_grad():
        ...
        if 'cifar' in dataset_name:
            ...
            # Tiny ImageNet (or others) uses ImageFolder: extract labels from samples
            feature_labels = torch.tensor([s[1] for s in memory_data_loader.dataset.samples], device=feature_bank.device)
...
if __name__ == '__main__':
    ...
    # Changed default dataset name to tiny_imagenet.
    ...
    # Changed default dataset location to Tiny ImageNet path.
